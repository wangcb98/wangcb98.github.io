%% Main body
function [pred_pm2d5,interpo_pred_clean] = pm2d5_pred_model(train_data, test_data, problem_type)
...
[train_data_sampled] = movAvg(train_data,problem_type); % Time is in minutes
...
switch problem_type
    case 1 % Short Term
        ...
        weight = 0.6*prelim_weight + 0.4*rmse_weight;
...
%         end
    ...
    case 2 % Long Term
        ...
        long_term_pred = [pred_pm2d5_simplelinear,pred_pm2d5_simplelinearlasso,...
            pred_pm2d5_polylasso,pred_pm2d5_sineridge,pred_pm2d5_sinelasso,...
            pred_pm2d5_gpr];
        rmse = [rmse_simplelinear;rmse_simplelinearlasso;...
            rmse_polylasso;rmse_sineridge;rmse_sinelasso;...
            rmse_gpr];
...
%         end
        ...
        weight
        long_term_pred_clean
        pred_pm2d5 = long_term_pred_clean*weight;
        %% ONLY FOR PLOTTING:
        %save('modelInfo.mat','long_term_pred','rmse','test_data');
...
%% movAvg
function [train_data_sampled] = movAvg(train_data,problem_type)
...
% depending on the problem type, the tuned lambda is different
lam = [0.1,0.1];
...
[beta, FitInfo] = lasso(X_train, Y_train, 'Lambda',lambda);
intercept = FitInfo.Intercept;
pm2d5_pred = X_test(:,2:dim+1)*beta + intercept*ones(length(X_test(:,1)),1);
X_test_CV = [ones(length(test_CV(:,1)),1),test_CV(:,[1,2,3,4,6,7])];
pm2d5_CV = X_test_CV(:,2:dim+1)*beta + intercept*ones(length(X_test_CV(:,1)),1);
...
elseif problem_type == 3
    ...
    for i=1:length(t)
        ...
        lat_expand((5*i-4):5*i,1) = test_data.lat(i);
        lon_expand((5*i-4):5*i,1) = test_data.lon(i);
    end  
else
...
X = [ones(length(train_data_sampled(:,1)),1),sin(train_data_sampled(:,1)*omega),...
    cos(train_data_sampled(:,1)*omega),train_data_sampled(:,6),...
...
pm2d5_CV = [ones(length(test_CV),1),sin(test_CV(:,1)*omega),cos(test_CV(:,1)*omega),...
    ...
    test_CV(:,7), test_CV(:,7).^2, test_CV(:,7).^3]*beta;
...
for i = 1:5
    ...
    [~,err] = fun(train,test_CV,test_data,t0,problem_type);
    rmse = rmse + err;
end
rmse = rmse/5;
[pm2d5_pred,~] = fun(train_data_sampled,test_CV,test_data,t0,problem_type);
end
